...
const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
        ...
        if (mutation.type === "childList") {
            if (mutation.addedNodes.length > 0) {
                ...
                mutation.addedNodes.forEach((node) => {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        console.log(
                            `  → Added element: <${node.tagName?.toLowerCase()}>`
                        );
            ...
            if (mutation.removedNodes.length > 0) {
                ...
                mutation.removedNodes.forEach((node) => {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        console.log(
                            `  → Removed element: <${node.tagName?.toLowerCase()}>`
                        );
                    } else if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (mutation.type === "attributes") {
            ...
            console.log(
                `New value: ${mutation.target.getAttribute(
        ...
        if (mutation.type === "characterData") {
            console.log(
                `Text content changed to: "${mutation.target.textContent}"`
            );
        }
...
console.log(
    "🎯 DOM Mutation Observer started - React's imperative DOM updates will be logged below:"
);
console.log("────────");
